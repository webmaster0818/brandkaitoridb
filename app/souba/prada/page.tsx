...
export const metadata: Metadata = {
  title: "プラダの実売相場データ｜買取相場の目安【2026年8月取得】",
  description:
    "プラダのバッグ・財布について、オークションで実際に売買が成立した価格を集計。直近30日の件数・平均落札価格・最高値を出典つきで公開します。査定前の物差しにどうぞ。",
  alternates: { canonical: `${SITE_URL}/souba/prada/` },
};
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 3, name: "プラダ", item: `${SITE_URL}/souba/prada/` },
  ],
};
...
const rows = [
  {
    ...
    avg: "45,702円",
    max: "147,000円",
    sourceUrl: "https://aucfan.com/search1/q-~a5d7a5e9a5c020a5d0a5c3a5b0/s-ya/",
  },
  {
    category: "財布",
    count: "755件",
    avg: "19,773円",
    max: "101,000円",
    sourceUrl: "https://aucfan.com/search1/q-~a5d7a5e9a5c020bae2c9db/s-ya/",
  },
];

const pendingCategories = ["ポーチ・小物"];

export default function PradaSoubaPage() {
  return (
    <main>
      <script
      ...
      <section className="border-b border-line">
        <div className="mx-auto max-w-6xl px-5 pt-10 pb-12 md:pt-14 md:pb-16">
          <Breadcrumbs
            items={[{ label: "実売相場", href: "/souba/" }, { label: "プラダ" }]}
          />
          <p className="eyebrow mt-8 mb-4">MARKET DATA — PRADA</p>
          ...
          <p className="mt-5 max-w-2xl text-[0.95rem] leading-loose text-ink-soft">
            業者の広告に載る「参考買取価格」ではなく、オークションで実際に売買が成立した価格だけを集計しました。プラダはバッグだけで直近30日に2,000件超の落札が確認できるほど中古市場での取引が活発なブランドです。査定に出す前の「相場の物差し」としてお使いください。
          </p>
        ...
        <div className="mx-auto max-w-6xl px-5 py-14 md:py-18">
          ...
          <div className="max-w-3xl space-y-5 text-[0.92rem] leading-loose text-ink-soft">
            <p>
              ...
              集計には状態不良品・部品取り目的・複数点のまとめ売りなど、極端な安値の取引も含まれます。お手元の品物の状態が良ければ、平均より上のレンジが目安になります。
            </p>
            <p>
              <span className="font-bold text-ink">高値側には特殊な取引が含まれます。</span>
              最高落札価格は未使用品・人気型番・付属品完備といった好条件のケースであることが多く、1点の通常品の上限とは限りません。
            </p>
            <p>
              <span className="font-bold text-ink">同じ「プラダのバッグ」でも素材・ラインで価格は大きく変わります。</span>
