...
export const metadata: Metadata = {
  title: "セリーヌの実売相場データ｜買取相場の目安【2026年8月取得】",
  description:
    "セリーヌのバッグ・財布について、オークションで実際に売買が成立した価格を集計。直近30日の件数・平均落札価格・最高値を出典つきで公開します。査定前の物差しにどうぞ。",
  alternates: { canonical: `${SITE_URL}/souba/celine/` },
};
...
const breadcrumbJsonLd = {
  ...
  itemListElement: [
    ...
    { "@type": "ListItem", position: 3, name: "セリーヌ", item: `${SITE_URL}/souba/celine/` },
  ],
};
...
const rows = [
  {
    ...
    avg: "39,432円",
    max: "148,000円",
    sourceUrl: "https://aucfan.com/search1/q-~a5bba5eaa1bca5cc20a5d0a5c3a5b0/s-ya/",
  },
  {
    category: "財布",
    count: "310件",
    avg: "24,722円",
    max: "148,000円",
    sourceUrl: "https://aucfan.com/search1/q-~a5bba5eaa1bca5cc20bae2c9db/s-ya/",
  },
];

const pendingCategories = ["アクセサリー"];

export default function CelineSoubaPage() {
  return (
    <main>
      <script
      ...
      <section className="border-b border-line">
        <div className="mx-auto max-w-6xl px-5 pt-10 pb-12 md:pt-14 md:pb-16">
          <Breadcrumbs
            items={[{ label: "実売相場", href: "/souba/" }, { label: "セリーヌ" }]}
          />
          <p className="eyebrow mt-8 mb-4">MARKET DATA — CELINE</p>
          ...
          <p className="mt-5 max-w-2xl text-[0.95rem] leading-loose text-ink-soft">
            業者の広告に載る「参考買取価格」ではなく、オークションで実際に売買が成立した価格だけを集計しました。セリーヌは現行ラインからヴィンテージ（オールドセリーヌ）まで中古市場での取引が安定して多いブランドです。査定に出す前の「相場の物差し」としてお使いください。
          </p>
      ...
      <section className="bg-ivory-deep border-y border-line">
        <div className="mx-auto max-w-6xl px-5 py-14 md:py-18">
          ...
          <div className="max-w-3xl space-y-5 text-[0.92rem] leading-loose text-ink-soft">
            ...
            <p>
              <span className="font-bold text-ink">同じ「セリーヌのバッグ」でもライン・年代で価格は大きく変わります。</span>
      ...
      <section className="mx-auto max-w-6xl px-5 py-14 md:py-18">
        ...
        <div className="max-w-3xl space-y-4 text-[0.92rem] leading-loose text-ink-soft">
          <p>
            箱・保存袋・レシートなどの付属品は、揃っているだけで査定の条件が変わります。購入時の付属品を先に探しておきましょう。バッグや財布の内側にある型番・製造刻印を控えておくと、実売データとの照合もしやすくなります。
          </p>
          <p>
